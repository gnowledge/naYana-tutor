...
test('phase 8 catalogue extends through the long-u rules', () => {
  ...
  assert.ok(names.includes('o→u (when /uː/)'));
  assert.ok(names.includes('oo→u (when /uː/)'));
  assert.ok(names.includes('ew→u (when /uː/)'));
  assert.ok(names.includes('ou→u (when /uː/)'));
  assert.ok(names.includes('ui→u (when /uː/)'));
  assert.ok(names.includes('w→u (when /uː/)'));
  assert.equal(phase8Rules.length, 30, 'rule count = 24 prior + 6 new');
});

// ---- o → u (when /uː/) ----------------------------------------------------

test('do → du', () => {
  assert.equal(rewrite('do').spelling, 'du');
});

test('to → tu', () => {
  assert.equal(rewrite('to').spelling, 'tu');
});

test('move → muv (o→u AND silent-e)', () => {
  assert.equal(rewrite('move').spelling, 'muv');
});
...
test('lose → lus (o→u AND silent-e; s stays s pending future s→z rule)', () => {
  ...
  assert.equal(rewrite('lose').spelling, 'lus');
});
...
test('food → fud', () => {
  assert.equal(rewrite('food').spelling, 'fud');
});

test('moon → mun', () => {
  assert.equal(rewrite('moon').spelling, 'mun');
});
...
test('soon → sun', () => {
  ...
  assert.equal(rewrite('soon').spelling, 'sun');
});

test('spoon → spun', () => {
  assert.equal(rewrite('spoon').spelling, 'spun');
});
...
test('new → nu', () => {
  assert.equal(rewrite('new').spelling, 'nu');
});

test('blew → blu', () => {
  assert.equal(rewrite('blew').spelling, 'blu');
});

test('flew → flu', () => {
  assert.equal(rewrite('flew').spelling, 'flu');
});

test('CRITICAL: knew → nu (kn→n AND ew→u; true homophone with new)', () => {
  // Both new and knew become 'nu' — the spelling reform reveals what
  // was always a true homophone in spoken English.
  assert.equal(rewrite('knew').spelling, 'nu');
});
...
test('group → grup', () => {
  assert.equal(rewrite('group').spelling, 'grup');
});

test('soup → sup', () => {
  assert.equal(rewrite('soup').spelling, 'sup');
});
...
test('fruit → frut', () => {
  assert.equal(rewrite('fruit').spelling, 'frut');
});

test('suit → sut', () => {
  assert.equal(rewrite('suit').spelling, 'sut');
});

test('juice → jus (ui→u AND c→s AND silent-e)', () => {
  // alignment: j}JH ui}UW1 c}S e}∅ → j + u + s + "" = "jus"
  assert.equal(rewrite('juice').spelling, 'jus');
});
...
test('few → fu (silent-e from phase 6 AND w→u)', () => {
  ...
  assert.equal(rewrite('few').spelling, 'fu');
});

test('anew → anu', () => {
  assert.equal(rewrite('anew').spelling, 'anu');
});

// ---- u + silent-e (already handled by phase 6) ----------------------------

test('blue → blu (silent-e from phase 6, no new rule needed)', () => {
  assert.equal(rewrite('blue').spelling, 'blu');
});

test('true → tru (silent-e from phase 6)', () => {
  assert.equal(rewrite('true').spelling, 'tru');
});

test('rule → rul (silent-e from phase 6)', () => {
  assert.equal(rewrite('rule').spelling, 'rul');
});

test('flute → flut (silent-e from phase 6)', () => {
  assert.equal(rewrite('flute').spelling, 'flut');
});
